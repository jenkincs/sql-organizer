...
describe('OpenAI endpoint support', () => {
  it('normalizes safe base URLs and rejects unsafe forms', () => {
    expect(normalizeBaseUrl('https://llm.example.test/v1/')).toBe('https://llm.example.test/v1');
    expect(() => normalizeBaseUrl('https://key@llm.example.test/v1')).toThrow();
    expect(() => normalizeBaseUrl('https://llm.example.test/v1?token=x')).toThrow();
  });
  it('uses Responses output_text with storage disabled', async () => {
    let request: unknown;
    const client = {
      responses: {
    ...
    const provider = new OpenAiProvider(
      { apiKey: 'test', model: 'model', timeoutMs: 1, protocol: 'responses' },
      client as never,
    );
  ...
  it('uses Chat Completions when explicitly selected', async () => {
    ...
    const provider = new OpenAiProvider(
      { apiKey: 'test', model: 'model', timeoutMs: 1, protocol: 'chat-completions' },
      client as never,
    );
    await expect(provider.classify(input)).resolves.toMatchObject(valid);
