...
export interface OpenAiProviderOptions { apiKey: string; model: string; timeoutMs: number; baseUrl?: string; protocol?: OpenAiProtocol; }
const systemPrompt = 'You are SQL Librarian. Return only a JSON object with category, operation, dialect, purpose, suggestedFilename, tables, parameters, risk, riskReasons, confidence and reviewNotes. Never execute SQL or recommend deletion.';
...
export function normalizeBaseUrl(value?: string): string | undefined {
  ...
  let url: URL;
  try { url = new URL(value.trim()); } catch { throw new Error('AI base URL must be a valid http(s) URL.'); }
  if (!['https:', 'http:'].includes(url.protocol) || url.username || url.password || url.search || url.hash) throw new Error('AI base URL must use http(s) and must not contain credentials, query parameters, or fragments.');
...
export class OpenAiProvider implements AiProvider {
  ...
  constructor(private readonly options: OpenAiProviderOptions, client?: OpenAI) {
    this.protocol = options.protocol ?? 'responses';
    this.client = client ?? new OpenAI({ apiKey: options.apiKey, baseURL: normalizeBaseUrl(options.baseUrl), timeout: options.timeoutMs, maxRetries: 0 });
  }
  async classify(input: ClassificationInput): Promise<SqlClassification> {
    if (!this.options.model) throw new Error('OpenAI model is not configured. Set sqlOrganizer.model and retry Analyze.');
  ...
  private async classifyWithChatCompletions(input: ClassificationInput): Promise<string> {
    const response = await this.client.chat.completions.create({ model: this.options.model, response_format: { type: 'json_object' }, messages: [{ role: 'system', content: systemPrompt }, { role: 'user', content: JSON.stringify(input) }] });
