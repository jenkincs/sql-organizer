...
function html(
  ...
  const approvedCount = plan.actions.filter((action) => action.status === 'approved').length;
  const reviewableCount = plan.actions.filter(
    (action) => !action.validationErrors.length && action.status !== 'conflict' && action.status !== 'rejected',
  ).length;
