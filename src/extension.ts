...
export function activate(context: vscode.ExtensionContext): void {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('sqlOrganizer.analyze', async () => { const folder = root(); if (!folder) return void vscode.window.showErrorMessage('Open a workspace folder before analyzing SQL files.'); const config = await loadConfig(folder); const settings = vscode.workspace.getConfiguration('sqlOrganizer'); const key = await context.secrets.get('sqlOrganizer.openaiApiKey'); if (!key) return void vscode.window.showErrorMessage('OpenAI API key is not configured. Run “SQL Organizer: Set OpenAI API Key” and retry Analyze.'); await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'SQL Organizer: Analyzing SQL files', cancellable: true }, async (_, token) => { const repo = new Repository(folder, config); const provider = new OpenAiProvider({ apiKey: key, model: config.ai.model || settings.get('model', ''), baseUrl: config.ai.baseUrl || settings.get('baseUrl', ''), protocol: config.ai.apiProtocol || settings.get('apiProtocol', 'responses'), timeoutMs: config.ai.timeoutMs }); await new ClassificationService(folder, config, repo, provider).analyze(await repo.inventory(), token); status.text = '$(database) SQL Organizer: Analysis complete'; refresh(); }); }));
